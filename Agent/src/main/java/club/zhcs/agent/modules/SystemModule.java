...
import club.zhcs.agent.model.biz.shiro.ShiroUserService;
...
public class SystemModule extends BaseModule {
	...
	public Result main(@Attr(SessionKeys.USER_KEY) User user) {
		return Result.success().setTitle("项目说明");
	}
	...
	public Result save(@Param("realName") String realName, @Param("phone") String phone, @Param("email") String email, @Attr(SessionKeys.USER_KEY) User user) {
		return userService.update(Chain.make("realName", realName).add("phone", phone).add("email", email), Cnd.where("id", "=", user.getId())) == 1 ? Result.success()
				: Result.fail("更新失败!");
	}
